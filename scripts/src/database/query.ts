import { exec, ErrorCallback, getValueType, isInteger, SuccessCallback } from '../utils';
...
export class Query {
    ...
    private _attachListener(once: boolean = false, eventType: string = 'value', callback: EventListenerCallback, errorFn?: (err?: Error) => any) {
        ...
        exec(
            ...
            error,
            'Firebase',
            'database_attachListener',
            [listenerID, once, this._path.toString(), eventType, queryOptions],
        );
    }
    ...
    off(eventType?: string, callback?: EventListenerCallback): Promise<void> {
        return new Promise<void>((resolve: SuccessCallback, reject: ErrorCallback) => {
            ...
            exec(success, reject, 'Firebase', 'database_removeListeners', [listenerIDs]);
        });
    }

    keepSynced(keepSynced: boolean): Promise<void> {
        return new Promise<void>((resolve: SuccessCallback, reject: ErrorCallback) => {
            exec(() => resolve(), reject, 'Firebase', 'database_keepSynced', [this._path.toString(), keepSynced]);
        });
    }
